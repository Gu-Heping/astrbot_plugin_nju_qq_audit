...
- 仅姓名+硕/博命中的研究生申请不会触发 auto 自动通过，会默认进入 release 队列并继续通知管理员；通知展示名单专业，便于管理员在 release 前 `/audit no` 拒绝。
- 优化研究生复盘报告的名单辅助解析，避免 `/audit report grad` 对每条历史申请重复扫描整份研究生名单。
...
- 新增本科多群互斥检查：防止同一 QQ 加入多个本科目标群；命中时仅转人工，不自动拒绝。
...
- 入群事件透传 `unified_msg_origin`（umo）以解析会话级 LLM provider
- AstrBot `llm_generate` 不再因 TypeError 二次重试（避免重复发送申请内容）
- AI 异常日志脱敏，避免泄露 AstrBot provider 侧 API key / Authorization
...
- 本科审核：支持「姓名+学号/录取号+专业」中使用 `+` / `＋` / `/` / `／` 作为分隔符
- 不改变自动通过门槛，仍由姓名+学号/通知书编号强匹配决定
...
- external 通知解析：优先从「答案：/答:/回答：/A:/answer:」抽取填写内容，再切 token，避免把问题模板当成申请人
...
- 首页健康判断同时考虑本科与研究生目标群；仅启用研究生时不再误报「需要配置」
- 区分「未配置任何目标群」与「研究生已启用但未配置研究生目标群」提示
