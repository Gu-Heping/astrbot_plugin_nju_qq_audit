...
def test_fullwidth_plus_separator():
    parsed = parse_application_comment("周七七＋261880001＋技术科学试验班")
    assert parsed.name == "周七七"
    assert parsed.student_id == "261880001"
    assert parsed.major == "技术科学试验班"
...
def test_space_and_comma_still_work():
    spaced = parse_application_comment("周七七 261880001 技术科学试验班")
    assert spaced.name == "周七七"
    assert spaced.student_id == "261880001"
    assert spaced.major == "技术科学试验班"

    comma = parse_application_comment("周七七，261880001，技术科学试验班")
    assert comma.name == "周七七"
    assert comma.student_id == "261880001"
    assert comma.major == "技术科学试验班"


def test_name_plus_student_id_only():
    parsed = parse_application_comment("周七七+261880001")
    assert parsed.name == "周七七"
    assert parsed.student_id == "261880001"
