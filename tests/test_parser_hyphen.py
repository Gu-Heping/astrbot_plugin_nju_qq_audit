...
def test_hyphen_separated_name_sid_major():
    ...
    assert parsed.name == "吴九九"
    assert parsed.student_id == "261220028"
...
def test_hyphen_variants_fullwidth_and_dash():
    for sep in ("-", "－", "—", "–"):
        parsed = parse_application_comment(f"吴九九{sep}261220028{sep}环境与健康实验班")
        assert parsed.name == "吴九九", sep
        assert parsed.student_id == "261220028", sep
